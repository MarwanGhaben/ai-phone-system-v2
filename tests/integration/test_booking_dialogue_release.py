...
spec = importlib.util.spec_from_file_location("booking_dialogue_repair", ROOT / "scripts/deploy-booking-dialogue-repair.py")
...
class Harness:
    ...
    def inspect_once(self, name):
        if name.startswith("ai-phone-booking-dialogue-repair-parent:"):
            return {"Id": "wrong" if self.fault == "parent" else TEST_IMAGE}
        if name.startswith("ai-phone-booking-dialogue-repair:"):
    ...
    def run(self, *args, **kwargs):
        ...
        if args[0] == "git":
            ...
            value = b"BOOKING_DIALOGUE_OFFLINE_DAY_LANGUAGE_OK"
        else:
            value = b""
...
def test_success_changes_only_app_image_and_build_has_no_private_data(tmp_path, monkeypatch):
    ...
    assert set(p.name for p in (h.directory / "build").iterdir()) == {"Dockerfile", "spoken_arabic.py", "proposals.py", "orchestrator.py", "booking_dialogue.py", "booking_session.py", "tool_protocol.py"}
...
def test_preparation_failure_does_not_stop_app(tmp_path, monkeypatch, fault):
    ...
    if fault in ("parent", "build"):
        assert ("docker", "image", "rm", "ai-phone-booking-dialogue-repair-parent:" + h.directory.name) in h.events
    if fault == "parent":
...
@pytest.mark.skipif(os.environ.get("BOOKING_DIALOGUE_REPAIR_RUN_DOCKER_TESTS") != "1",
...
def test_actual_build_and_parser_probe_in_linux(tmp_path, monkeypatch):
    ...
    parent_name = os.environ.get("BOOKING_DIALOGUE_REPAIR_TEST_PARENT", "ai-phone-t049a-candidate:20260913")
    ...
    base_tag = "ai-phone-booking-dialogue-test-base:" + directory.name
    prepared_tag = "ai-phone-booking-dialogue-test-prepared:" + directory.name
    candidate_tag = "ai-phone-booking-dialogue-repair:" + directory.name
    parent_tag = "ai-phone-booking-dialogue-repair-parent:" + directory.name
    ...
    try:
        ...
        assert b"BOOKING_DIALOGUE_OFFLINE_DAY_LANGUAGE_OK" in result.stdout
