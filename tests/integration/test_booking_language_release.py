...
spec = importlib.util.spec_from_file_location("booking_language_repair", ROOT / "scripts/deploy-booking-language-repair.py")
...
class Harness:
    ...
    def inspect_once(self, name):
        if name.startswith("ai-phone-booking-language-repair-parent:"):
            return {"Id": "wrong" if self.fault == "parent" else TEST_IMAGE}
        if name.startswith("ai-phone-booking-language-repair:"):
    ...
    def run(self, *args, **kwargs):
        ...
        if args[0] == "git":
            ...
            value = b"BOOKING_LANGUAGE_OFFLINE_CANONICAL_SWITCH_AVAILABILITY_OK"
        else:
            value = b""
...
def test_success_changes_only_app_image_and_build_has_no_private_data(tmp_path, monkeypatch):
    ...
    assert set(p.name for p in (h.directory / "build").iterdir()) == {
        "Dockerfile", "booking_dialogue.py", "language_policy.py", "orchestrator.py"}
...
def test_preparation_failure_does_not_stop_app(tmp_path, monkeypatch, fault):
    ...
    if fault in ("parent", "build"):
        assert ("docker", "image", "rm", "ai-phone-booking-language-repair-parent:" + h.directory.name) in h.events
    if fault == "parent":
...
@pytest.mark.skipif(os.environ.get("BOOKING_LANGUAGE_REPAIR_RUN_DOCKER_TESTS") != "1",
...
def test_actual_build_and_parser_probe_in_linux(tmp_path, monkeypatch):
    ...
    parent_name = os.environ.get("BOOKING_LANGUAGE_REPAIR_TEST_PARENT", "ai-phone-t049a-candidate:20260913")
    ...
    base_tag = "ai-phone-booking-language-test-base:" + directory.name
    prepared_tag = "ai-phone-booking-language-test-prepared:" + directory.name
    candidate_tag = "ai-phone-booking-language-repair:" + directory.name
    parent_tag = "ai-phone-booking-language-repair-parent:" + directory.name
    ...
    try:
        ...
        assert b"BOOKING_LANGUAGE_OFFLINE_CANONICAL_SWITCH_AVAILABILITY_OK" in result.stdout
