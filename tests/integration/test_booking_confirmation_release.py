...
spec = importlib.util.spec_from_file_location("booking_confirmation_repair", ROOT / "scripts/deploy-booking-confirmation-repair.py")
...
class Harness:
    ...
    def inspect_once(self, name):
        if name.startswith("ai-phone-booking-confirmation-repair-parent:"):
            return {"Id": "wrong" if self.fault == "parent" else TEST_IMAGE}
        if name.startswith("ai-phone-booking-confirmation-repair:"):
    ...
    def run(self, *args, **kwargs):
        ...
        if args[0] == "git":
            ...
            value = b"BOOKING_CONFIRMATION_OFFLINE_MARK_AND_CONSENT_OK"
        else:
            value = b""
...
def test_success_changes_only_app_image_and_build_has_no_private_data(tmp_path, monkeypatch):
    ...
    assert set(p.name for p in (h.directory / "build").iterdir()) == {
        "Dockerfile", "booking_session.py", "orchestrator.py", "twilio_service.py"}
...
def test_preparation_failure_does_not_stop_app(tmp_path, monkeypatch, fault):
    ...
    if fault in ("parent", "build"):
        assert ("docker", "image", "rm", "ai-phone-booking-confirmation-repair-parent:" + h.directory.name) in h.events
    if fault == "parent":
...
@pytest.mark.skipif(os.environ.get("BOOKING_CONFIRMATION_REPAIR_RUN_DOCKER_TESTS") != "1",
                   reason="opt-in offline Linux image build and canonical STT and language-switch probe")
def test_actual_build_and_parser_probe_in_linux(tmp_path, monkeypatch):
    ...
    parent_name = os.environ.get("BOOKING_CONFIRMATION_REPAIR_TEST_PARENT", "ai-phone-t049a-candidate:20260913")
    ...
    base_tag = "ai-phone-booking-confirmation-test-base:" + directory.name
    prepared_tag = "ai-phone-booking-confirmation-test-prepared:" + directory.name
    candidate_tag = "ai-phone-booking-confirmation-repair:" + directory.name
    parent_tag = "ai-phone-booking-confirmation-repair-parent:" + directory.name
    ...
    try:
        ...
        assert b"BOOKING_CONFIRMATION_OFFLINE_MARK_AND_CONSENT_OK" in result.stdout
