"""Owner-run app-only duplicate-availability repair and private create diagnostics; never runs a migration."""
import copy
import hashlib
import json
import os
...
BASE = '3e492696348c4c4ce617d69b6b7cd817f56d7127'
ROOT = Path("/opt/ai-phone-system-v2")
SOURCES = {'services/calendar/contracts.py': 'aefb4279d7c07429fe497abd92318e1e6daa9104bcc8ed775a96e3eebb88e639', 'services/calendar/booking_mutations.py': '6d58c7a5208d1269d59afe9958c9d2c26d7eee467c6c1bc6ff1fa76f07091a1b', 'services/scheduling/booking_service.py': '5d8a7a062daf7946d9848e41b07b93e49c499bd4a556396f47c5414b11462f9d'}
SUPPORTED_PREDECESSORS = {'3e492696348c4c4ce617d69b6b7cd817f56d7127': {'services/calendar/booking_mutations.py': '3ee1334812ff96563da5bbe68ecadd776243b6a689399811880a8952ab39d023', 'services/calendar/contracts.py': '7b62912d0304849494597cad4323bab9c6b6303d41485a866762fd43b20d2a96', 'services/calendar/service_facts.py': '3e1c3a6977e9d008495e6bb312713b5544ed3fd03108597e61d520b3d53a6822', 'services/conversation/booking_dialogue.py': 'b787a10e789973a6fe0071ec122b72686e69b54a430c08dfbfa7663bbeb1c1af', 'services/conversation/booking_session.py': '99e5644ecaec817055816f07941bce43ee051d0d60d507872ecbe1a92ffe0387', 'services/conversation/orchestrator.py': '00d3018e2dee423a0cf50e575fa10c4ec0f1da2fce1cdb4d30b21c13feaceb33', 'services/llm/openai_service.py': '333acc9e5e022d2341fcd9b07df46927eec34bd195c86abe3711073523468d61', 'services/llm/tool_protocol.py': '82054d4cab596aec27d81b7e1278737c10ea4fdf0cd4bfe4b23dfba6da8b06fd', 'services/scheduling/booking_service.py': '68d19d5c0fe3311371cada2f4d4c0d1b2c86095721673dc14aa09a925c5f5534', 'services/scheduling/proposals.py': 'df52bb23abd8f7855b6edb7e2b38a79c7cf13aae273c860cdc9f3369800c103b', 'services/scheduling/spoken_arabic.py': 'a11ad2a8bbd6e8c57e964fe717c37f7037b8d206c1b4e3d4d8c189ca3275d130'}}
PROBE_PATH = "scripts/probe-booking-incident-repair.py"
PROBE_SHA = 'd6a9f0f125c8dced00be9deb7049e82e1e6b266e4facdda8eb85417029a5ca16'
...
def build_candidate(release, sources, old_image):
    ...
    parent = "ai-phone-booking-incident-repair-parent:" + release.directory.name
    tag = "ai-phone-booking-incident-repair:" + release.directory.name
    try:
...
def execute(release, helper):
    ...
    def probe(image_hashes, override=None):
        ...
        if override is None:
            ...
            name = "booking-incident-repair-probe-" + release.directory.name
            try:
    ...
    print("BOOKING_INCIDENT_REPAIR_BASELINE_SOURCE_SETTINGS_READY_OK", flush=True)
    release.stage = "build"
    candidate = build_candidate(release, sources, old_image)
    print("BOOKING_INCIDENT_REPAIR_LOCAL_PARENT_BUILD_OK", flush=True)
    release.stage = "incident-probe"
    name = "booking-incident-repair-parser-" + release.directory.name
    try:
        ...
        if b"BOOKING_INCIDENT_OFFLINE_AVAILABILITY_DIAGNOSTICS_OK" not in result.stdout:
    ...
    print("BOOKING_INCIDENT_REPAIR_CANDIDATE_AVAILABILITY_AND_EFFECTIVE_SOURCE_OK", flush=True)
    ...
    try:
        ...
        try:
            ...
            print("BOOKING_INCIDENT_REPAIR_PREVIOUS_APP_RESTORED", flush=True)
        except BaseException:
            release.force_close_ingress()
            print("BOOKING_INCIDENT_REPAIR_RECOVERY_UNVERIFIED", flush=True)
        raise
    release.mark("deployed", (release.commit + "\n" + candidate + "\n").encode())
    print("BOOKING_INCIDENT_REPAIR_DEPLOYED_READY_HTTPS_OK", flush=True)
...
def main(commit):
    ...
    with os.fdopen(fd, "r+") as lock:
        ...
        directory = Path(tempfile.mkdtemp(prefix="ai-phone-booking-incident-repair.", dir="/opt"))
        ...
        try:
            ...
            print("BOOKING_INCIDENT_REPAIR_STOPPED_STAGE=" + release.stage, flush=True)
            raise
...
if __name__ == "__main__":
    try:
        ...
        print("BOOKING_INCIDENT_REPAIR_STOPPED_KEEP_PROTECTED_FILES", flush=True)
