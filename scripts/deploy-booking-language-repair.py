"""Owner-run app-only caller-language repair; never runs a migration."""
import copy
import hashlib
import json
import os
...
BASE = '5a22b1b1713156e888342e65cc56ea3e22183704'
ROOT = Path("/opt/ai-phone-system-v2")
SOURCES = {'services/conversation/booking_dialogue.py': 'bc1afb8a0d3a3b72bd0c4e5070b0519877e3ed53b9c0f3956565ad225bc6e01a', 'services/conversation/language_policy.py': 'a50e8ec58737dd896686dccb4c52c427f94477d4898c2a17a9af74fdf44f8b9b', 'services/conversation/orchestrator.py': '4be45360bd7900946357a39a634f2f74d469fcc4bdf9a62f8aa25d5d88f51929'}
SUPPORTED_PREDECESSORS = {'5a22b1b1713156e888342e65cc56ea3e22183704': {'services/calendar/booking_mutations.py': '6d58c7a5208d1269d59afe9958c9d2c26d7eee467c6c1bc6ff1fa76f07091a1b', 'services/calendar/contracts.py': 'aefb4279d7c07429fe497abd92318e1e6daa9104bcc8ed775a96e3eebb88e639', 'services/calendar/service_facts.py': '3e1c3a6977e9d008495e6bb312713b5544ed3fd03108597e61d520b3d53a6822', 'services/conversation/booking_dialogue.py': 'b787a10e789973a6fe0071ec122b72686e69b54a430c08dfbfa7663bbeb1c1af', 'services/conversation/booking_session.py': '99e5644ecaec817055816f07941bce43ee051d0d60d507872ecbe1a92ffe0387', 'services/conversation/language_policy.py': '9ec8c2c349061ca39f3279fbfb9b89574e1a20d48e041eb800de1c1348ef39fa', 'services/conversation/orchestrator.py': '00d3018e2dee423a0cf50e575fa10c4ec0f1da2fce1cdb4d30b21c13feaceb33', 'services/llm/openai_service.py': '333acc9e5e022d2341fcd9b07df46927eec34bd195c86abe3711073523468d61', 'services/llm/tool_protocol.py': '82054d4cab596aec27d81b7e1278737c10ea4fdf0cd4bfe4b23dfba6da8b06fd', 'services/scheduling/booking_service.py': '5d8a7a062daf7946d9848e41b07b93e49c499bd4a556396f47c5414b11462f9d', 'services/scheduling/proposals.py': 'df52bb23abd8f7855b6edb7e2b38a79c7cf13aae273c860cdc9f3369800c103b', 'services/scheduling/spoken_arabic.py': 'a11ad2a8bbd6e8c57e964fe717c37f7037b8d206c1b4e3d4d8c189ca3275d130'}}
PROBE_PATH = "scripts/probe-booking-language-repair.py"
PROBE_SHA = '403d4e4a8c015d31c0fcbccf6765d5e21ea86023fd83ddecadfcd77670496e6e'
...
def build_candidate(release, sources, old_image):
    ...
    parent = "ai-phone-booking-language-repair-parent:" + release.directory.name
    tag = "ai-phone-booking-language-repair:" + release.directory.name
    try:
...
def execute(release, helper):
    ...
    def probe(image_hashes, override=None):
        ...
        if override is None:
            ...
            name = "booking-language-repair-probe-" + release.directory.name
            try:
    ...
    print("BOOKING_LANGUAGE_REPAIR_BASELINE_SOURCE_SETTINGS_READY_OK", flush=True)
    release.stage = "build"
    candidate = build_candidate(release, sources, old_image)
    print("BOOKING_LANGUAGE_REPAIR_LOCAL_PARENT_BUILD_OK", flush=True)
    release.stage = "language-probe"
    name = "booking-language-repair-parser-" + release.directory.name
    try:
        ...
        if b"BOOKING_LANGUAGE_OFFLINE_CANONICAL_SWITCH_AVAILABILITY_OK" not in result.stdout:
    ...
    print("BOOKING_LANGUAGE_REPAIR_CANDIDATE_LANGUAGE_AND_EFFECTIVE_SOURCE_OK", flush=True)
    ...
    try:
        ...
        try:
            ...
            print("BOOKING_LANGUAGE_REPAIR_PREVIOUS_APP_RESTORED", flush=True)
        except BaseException:
            release.force_close_ingress()
            print("BOOKING_LANGUAGE_REPAIR_RECOVERY_UNVERIFIED", flush=True)
        raise
    release.mark("deployed", (release.commit + "\n" + candidate + "\n").encode())
    print("BOOKING_LANGUAGE_REPAIR_DEPLOYED_READY_HTTPS_OK", flush=True)
...
def main(commit):
    ...
    with os.fdopen(fd, "r+") as lock:
        ...
        directory = Path(tempfile.mkdtemp(prefix="ai-phone-booking-language-repair.", dir="/opt"))
        ...
        try:
            ...
            print("BOOKING_LANGUAGE_REPAIR_STOPPED_STAGE=" + release.stage, flush=True)
            raise
...
if __name__ == "__main__":
    try:
        ...
        print("BOOKING_LANGUAGE_REPAIR_STOPPED_KEEP_PROTECTED_FILES", flush=True)
