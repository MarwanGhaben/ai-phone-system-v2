"""Owner-run app-only Arabic booking speech repair; never runs a migration."""
import copy
import hashlib
import json
import os
...
BASE = "20c917bb5a44b3ec596b8db1f50400218832d95a"
ROOT = Path("/opt/ai-phone-system-v2")
SOURCES = {'services/scheduling/spoken_arabic.py': 'a11ad2a8bbd6e8c57e964fe717c37f7037b8d206c1b4e3d4d8c189ca3275d130', 'services/scheduling/proposals.py': 'df52bb23abd8f7855b6edb7e2b38a79c7cf13aae273c860cdc9f3369800c103b', 'services/conversation/orchestrator.py': '40fc4cff4997fc6552621678483b3fe105e8d32f1cc860c2bcbdaf3a95b1caaf'}
PREDECESSOR_SOURCES = {'services/llm/openai_service.py': '333acc9e5e022d2341fcd9b07df46927eec34bd195c86abe3711073523468d61', 'services/calendar/contracts.py': '7b62912d0304849494597cad4323bab9c6b6303d41485a866762fd43b20d2a96', 'services/calendar/service_facts.py': '3e1c3a6977e9d008495e6bb312713b5544ed3fd03108597e61d520b3d53a6822', 'services/conversation/orchestrator.py': '9b60780d9b970251158ba8a19ddb52cc43eada3cf32f71d01a61be40ad774e0e'}
PROBE_PATH = "scripts/probe-arabic-booking-speech.py"
PROBE_SHA = 'b325a93c7d1c42fd01a00fb0fa40a1f6d7053b7bc730d2b3f774df82f3aacefe'
...
def build_candidate(release, sources, old_image):
    ...
    parent = "ai-phone-arabic-speech-repair-parent:" + release.directory.name
    tag = "ai-phone-arabic-speech-repair:" + release.directory.name
    try:
...
def execute(release, helper):
    ...
    for path, expected in PREDECESSOR_SOURCES.items():
        if digest(release.run("git", "show", BASE + ":" + path).stdout) != expected:
    ...
    def probe(image_hashes, override=None):
        ...
        if override is None:
            ...
            name = "arabic-speech-repair-probe-" + release.directory.name
            try:
    ...
    print("ARABIC_SPEECH_REPAIR_BASELINE_SOURCE_SETTINGS_READY_OK", flush=True)
    release.stage = "build"
    candidate = build_candidate(release, sources, old_image)
    print("ARABIC_SPEECH_REPAIR_LOCAL_PARENT_BUILD_OK", flush=True)
    release.stage = "speech-probe"
    name = "arabic-speech-repair-parser-" + release.directory.name
    try:
        ...
        if b"ARABIC_BOOKING_SPEECH_OFFLINE_PRESENTATION_CLAIM_OK" not in result.stdout:
            raise RuntimeError("Arabic speech probe did not complete")
    finally:
        release.run("docker", "rm", "-f", name, check=False)
    ...
    print("ARABIC_SPEECH_REPAIR_CANDIDATE_SPEECH_AND_EFFECTIVE_SOURCE_OK", flush=True)
    ...
    try:
        ...
        try:
            ...
            print("ARABIC_SPEECH_REPAIR_PREVIOUS_APP_RESTORED", flush=True)
        except BaseException:
            release.force_close_ingress()
            print("ARABIC_SPEECH_REPAIR_RECOVERY_UNVERIFIED", flush=True)
        raise
    release.mark("deployed", (release.commit + "\n" + candidate + "\n").encode())
    print("ARABIC_BOOKING_SPEECH_REPAIR_DEPLOYED_READY_HTTPS_OK", flush=True)
...
def main(commit):
    ...
    with os.fdopen(fd, "r+") as lock:
        ...
        directory = Path(tempfile.mkdtemp(prefix="ai-phone-arabic-speech-repair.", dir="/opt"))
        ...
        try:
            ...
            print("ARABIC_SPEECH_REPAIR_STOPPED_STAGE=" + release.stage, flush=True)
            raise
...
if __name__ == "__main__":
    try:
        ...
        print("ARABIC_SPEECH_REPAIR_STOPPED_KEEP_PROTECTED_FILES", flush=True)
