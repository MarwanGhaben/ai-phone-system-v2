"""Owner-run T033-F app-only numeric barge-in diagnostics rollout.
...
DEPLOYED_SOURCE = "2ba16c1aeea787bd8de6a89a65e24f571e7d14dd"
OLD_IMAGE = "sha256:eeaf0a9a177cfd3b3f4c37ec29426b39407f9415d644a55c46f8733cb686f622"
NGINX_HASH = "59ac9bdaa9a86c1022a573c9709faddf7e93a53015da8acfcc4330b3e288f3ab"
PROJECT = "ai-phone-system-v2"
MANIFEST_PATH = "docs/delegation/T033-F-accepted-hashes.json"
# Pin the same accepted manifest after Git's declared CRLF-to-LF normalization.
MANIFEST_SHA256 = "c606c818fcdcb9bd0fc64ff06087d8e8a6409e80b25819d00f6a569e805da18c"
REVIEWED_INPUT_HEAD = "3b062d435e7bfc907f7feda8bef5949e7ffd843c"
PUBLIC_HEALTH_URL = "https://aiagent.ghaben.ca:8443/health"
RELEASE_PATHS = (
    "scripts/deploy-barge-in-diagnostics.py",
    "scripts/read-barge-in-diagnostics.py",
    "tests/integration/test_barge_in_diagnostics_release.py",
    "tests/integration/test_barge_in_diagnostics_readout.py",
    "docs/operations/T033-F-diagnostics-rollout.md",
    "docs/delegation/T033-F-result.md",
)
...
def settings_for(previous: dict, *, candidate: bool) -> dict:
    ...
    if candidate:
        expected.update(DIAGNOSTIC_SETTINGS)
    return expected
...
def normalized_compose(
    ...
    *,
    ...
    """Remove only the two reviewed setting changes from comparison."""
    ...
    if candidate:
        for key, expected in DIAGNOSTIC_ENV.items():
            if str(environment.get(key)).lower() != expected:
                raise RuntimeError("diagnostic environment differs")
    ...
    if candidate:
        ...
        raise RuntimeError("previous settings mount is ambiguous")
    volumes = [
        mount
        for mount in volumes
        if not (
...
def hidden_runtime_paths(mounts: list[dict]) -> tuple[str, ...]:
    ...
    hidden = []
    for source_path in (*RUNTIME_PATHS, *PROTECTED_RUNTIME_PATHS):
...
def container_contract(
    ...
    *,
    ...
    """Runtime fields which must survive, minus the reviewed exceptions."""
    ...
    for item in config.get("Env") or []:
        ...
        if key in DIAGNOSTIC_ENV:
            overrides.setdefault(key, []).append(value if separator else "")
        else:
            environment.append(item)
    if candidate and any(
        [value.lower() for value in overrides.get(key, [])] != [expected]
        for key, expected in DIAGNOSTIC_ENV.items()
    ):
        raise RuntimeError("replacement diagnostic environment differs")
    return {
...
class Release:
    ...
    def verify_source(self) -> None:
        ...
        if len(changed) != len(SOURCE_SCOPE) or set(changed) != SOURCE_SCOPE:
            raise RuntimeError("release source exceeds reviewed diagnostics scope")
    ...
    def stage_assets(self) -> None:
        ...
        manifest_bytes = normalized_bytes(manifest_source)
        ...
        if (
            ...
            or set(manifest.get("protected_runtime", {}))
            != set(PROTECTED_RUNTIME_PATHS)
            or set(manifest.get("tests", {})) != set(TEST_PATHS)
            or set(manifest.get("test_support", {})) != set(TEST_SUPPORT_PATHS)
            or set(manifest.get("source_scope", ())) != SOURCE_SCOPE
            or manifest.get("candidate_setting_overrides") != DIAGNOSTIC_SETTINGS
        ):
        ...
        print("REVIEWED_DIAGNOSTICS_ASSETS_STAGED_36", flush=True)
    ...
    def _validate_settings(self, settings: dict, *, candidate: bool) -> None:
        ...
        if candidate:
            if settings != settings_for(self.previous_settings, candidate=True):
                raise RuntimeError("candidate effective settings differ")
        elif (
            settings.get("elevenlabs_stt_filter_background_audio", False) is not False
            or settings.get("barge_in_diagnostics_enabled", False) is not False
        ):
            raise RuntimeError("previous diagnostics or background filter state differs")

    def schema_matches(self, image: str = OLD_IMAGE) -> bool:
        name = "t033f-schema-" + self.directory.name
    ...
    def preflight(self) -> dict:
        ...
        if hidden_runtime_paths(old.get("Mounts", [])):
            raise RuntimeError("running mount hides reviewed diagnostics source")
        ...
        if hidden_runtime_paths(rendered["services"]["app"].get("volumes", [])):
            raise RuntimeError("effective mount hides reviewed diagnostics source")
        self.old = old
    ...
    def build_candidate(self) -> str:
        if (
            ...
            or not self.runtime_hashes
            or not self.protected_runtime_hashes
        ):
            raise RuntimeError("reviewed assets were not staged")
        parent = "ai-phone-t033f-parent:" + self.directory.name
        ...
        tag = "ai-phone-t033f-candidate:" + self.directory.name
        self.run(
            "docker",
            "build",
            "--network",
        ...
        probe = self.one_shot(
            "t033f-image-probe-" + self.directory.name,
            image,
            [
                "-c",
                source_probe_code(
                    {**self.runtime_hashes, **self.protected_runtime_hashes},
        ...
        if (
            value.get("hashes")
            != {**self.runtime_hashes, **self.protected_runtime_hashes}
            or value.get("settings", {}).get("barge_in_diagnostics_enabled")
            is not False
            or value.get("settings", {}).get(
                "elevenlabs_stt_filter_background_audio"
            )
            is not False
        ):
            raise RuntimeError("candidate image source differs")
        print("DIAGNOSTICS_CANDIDATE_IMAGE_HASHES_IMPORTS_OK", flush=True)
        return image

    def run_candidate_tests(self, image: str) -> None:
        mounts = (
            "type=bind,src="
            + str(self.assets / "tests")
            + ",dst=/audit/tests,readonly",
            "type=bind,src="
            + str(self.assets / "scripts")
            + ",dst=/audit/scripts,readonly",
            "type=bind,src="
            + str(self.assets / "docker-compose.yml")
            + ",dst=/audit/docker-compose.yml,readonly",
        )
        arguments = [
            "-m",
            "pytest",
        ...
        self.one_shot(
            "t033f-tests-" + self.directory.name,
            image,
            arguments,
            mounts=mounts,
            synthetic_env=True,
        ...
        print("DIAGNOSTICS_CANDIDATE_SPEECH_TESTS_PASSED", flush=True)

    def candidate_probe(self, override: Path, label: str) -> dict:
        name = "t033f-settings-" + label + "-" + self.directory.name
        try:
            result = self.compose(
                override,
                "run",
    ...
    def prepare_override(self, image: str, original: dict) -> Path:
        ...
        environment.update(DIAGNOSTIC_ENV)
        ...
        if len(settings_mounts) > 1:
            raise RuntimeError("private settings mount is ambiguous")
        volumes[:] = [
            mount
            for mount in volumes
            if not (
        ...
        if hidden_runtime_paths(rendered["services"]["app"].get("volumes", [])):
            raise RuntimeError("candidate mount hides reviewed diagnostics source")
        probe = self.candidate_probe(path, "candidate")
        if (
            probe.get("hashes")
            != {**self.runtime_hashes, **self.protected_runtime_hashes}
            or probe.get("settings")
            != settings_for(self.previous_settings, candidate=True)
        ):
            raise RuntimeError("candidate effective source or settings changed")
        print("DIAGNOSTICS_SETTINGS_MOUNTS_AND_SOURCE_VERIFIED", flush=True)
        return path
    ...
    def stop_components(self, *, strict: bool) -> None:
        ...
        try:
            ...
            running = ["unverified"]
        # A stop can report an error when a failed replacement already removed
        # or stopped the container. Final observed state is authoritative; an
        # inspection failure remains represented by the "unverified" sentinel.
    ...
    def install_override(self, source: Path) -> None:
        ...
        temporary = self.root / (".t033f-override-" + self.directory.name)
    ...
    def verify_replaced(self, image: str, revision: str, *, candidate: bool) -> None:
        ...
        if settings != settings_for(self.previous_settings, candidate=candidate):
    ...
    def recover(self) -> None:
        ...
        self.start_ingress()
        print("PREVIOUS_APP_RESTORED_DIAGNOSTICS_OFF_SCHEMA_0004_UNCHANGED", flush=True)
    ...
    def deploy(self, candidate: str, candidate_override: Path) -> None:
        ...
        print("PRE_CUTOVER_DIAGNOSTICS_FINGERPRINTS_OK", flush=True)
        ...
        try:
            ...
            print("BARGE_IN_DIAGNOSTICS_DEPLOYED_READY_HTTPS_OK", flush=True)
            print("DEPLOYED_COMMIT=" + self.commit, flush=True)
            print("DEPLOYED_IMAGE=" + candidate, flush=True)
        except BaseException:
            print("DIAGNOSTICS_CUTOVER_FAILED_RESTORING_PREVIOUS_APP", flush=True)
            try:
                self.recover()
...
def main(commit: str) -> None:
    ...
    with os.fdopen(descriptor, "r+") as lock:
        ...
        for previous in Path("/opt").glob("ai-phone-diagnostics-release.*"):
            if (previous / "cutover-started").exists() and not (
                previous / "deployed"
            ).exists():
                raise RuntimeError("prior diagnostics cutover needs review")
        directory = Path(
            tempfile.mkdtemp(prefix="ai-phone-diagnostics-release.", dir="/opt")
        )
