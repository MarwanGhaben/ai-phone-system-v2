"""Owner-run app-only restoration of the pre-phase-5 caller flow; never runs a migration."""
import copy
import hashlib
import json
import os
...
def changed_override(original, image):
    ...
    environment["VERIFIED_PHONE_BOOKING_ENABLED"] = "false"
    environment.pop("verified_phone_booking_enabled", None)
    return result
...
def restored_contract(previous):
    ...
    result["environment"] = sorted(
        [item for item in result["environment"]
         if item.split("=", 1)[0].lower() != "verified_phone_booking_enabled"]
...
def build_candidate(release, sources, old_image):
    ...
    parent = "ai-phone-restore-call-flow-parent:" + release.directory.name
    tag = "ai-phone-restore-call-flow:" + release.directory.name
    try:
...
def check_future_operations(release, override=None):
    if override is None:
        ...
        name = "restore-claim-check-" + release.directory.name
        try:
    ...
    if (type(result) is not dict or type(result.get("unresolved_future_operations")) is not int
        ...
        raise RuntimeError("unresolved future booking requires reconciliation before restoration")
...
def execute(release, helper):
    ...
    target_settings = {**settings, "verified_phone_booking_enabled": False}
    check_future_operations(release)
    sources = {path: release.run("git", "show", RESTORE + ":" + path).stdout for path in SOURCES}
    ...
    def probe(image_hashes, override=None, expected_settings=None):
        ...
        if override is None:
            ...
            name = "restore-call-flow-probe-" + release.directory.name
            try:
    ...
    print("CALL_FLOW_RESTORE_BASELINE_SOURCE_SETTINGS_READY_OK", flush=True)
    release.stage = "build"
    candidate = build_candidate(release, sources, old_image)
    print("CALL_FLOW_RESTORE_LOCAL_PARENT_BUILD_OK", flush=True)
    release.stage = "confirmation-probe"
    name = "restore-call-flow-parser-" + release.directory.name
    try:
        ...
        if b"RESTORED_CALL_FLOW_OFFLINE_OK" not in result.stdout:
    ...
    print("CALL_FLOW_RESTORE_CANDIDATE_CONFIRMATION_AND_EFFECTIVE_SOURCE_OK", flush=True)
    ...
    try:
        ...
        try:
            ...
            print("CALL_FLOW_RESTORE_PREVIOUS_APP_RESTORED", flush=True)
        except BaseException:
            release.force_close_ingress()
            print("CALL_FLOW_RESTORE_RECOVERY_UNVERIFIED", flush=True)
        raise
    release.mark("deployed", (release.commit + "\n" + candidate + "\n").encode())
    print("CALL_FLOW_RESTORE_DEPLOYED_READY_HTTPS_OK", flush=True)
...
def main(commit):
    ...
    with os.fdopen(fd, "r+") as lock:
        ...
        directory = Path(tempfile.mkdtemp(prefix="ai-phone-restore-call-flow.", dir="/opt"))
        ...
        try:
            ...
            print("CALL_FLOW_RESTORE_STOPPED_STAGE=" + release.stage, flush=True)
            raise
...
if __name__ == "__main__":
    try:
        ...
        print("CALL_FLOW_RESTORE_STOPPED_KEEP_PROTECTED_FILES", flush=True)
        raise SystemExit(1) from None
