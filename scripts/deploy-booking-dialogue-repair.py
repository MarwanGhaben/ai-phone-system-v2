"""Owner-run app-only requested-day and bilingual booking dialogue repair; never runs a migration."""
import copy
import hashlib
import json
import os
...
BASE = "a3d2d50aa39e6f7928b1f7dd7c2e31dfa06ed425"
ROOT = Path("/opt/ai-phone-system-v2")
SOURCES = {'services/conversation/booking_dialogue.py': 'b787a10e789973a6fe0071ec122b72686e69b54a430c08dfbfa7663bbeb1c1af', 'services/conversation/booking_session.py': '99e5644ecaec817055816f07941bce43ee051d0d60d507872ecbe1a92ffe0387', 'services/conversation/orchestrator.py': '00d3018e2dee423a0cf50e575fa10c4ec0f1da2fce1cdb4d30b21c13feaceb33', 'services/llm/tool_protocol.py': '82054d4cab596aec27d81b7e1278737c10ea4fdf0cd4bfe4b23dfba6da8b06fd', 'services/scheduling/proposals.py': 'df52bb23abd8f7855b6edb7e2b38a79c7cf13aae273c860cdc9f3369800c103b', 'services/scheduling/spoken_arabic.py': 'a11ad2a8bbd6e8c57e964fe717c37f7037b8d206c1b4e3d4d8c189ca3275d130'}
SUPPORTED_PREDECESSORS = {'20c917bb5a44b3ec596b8db1f50400218832d95a': {'services/llm/openai_service.py': '333acc9e5e022d2341fcd9b07df46927eec34bd195c86abe3711073523468d61', 'services/calendar/contracts.py': '7b62912d0304849494597cad4323bab9c6b6303d41485a866762fd43b20d2a96', 'services/calendar/service_facts.py': '3e1c3a6977e9d008495e6bb312713b5544ed3fd03108597e61d520b3d53a6822', 'services/conversation/orchestrator.py': '9b60780d9b970251158ba8a19ddb52cc43eada3cf32f71d01a61be40ad774e0e'}, 'a3d2d50aa39e6f7928b1f7dd7c2e31dfa06ed425': {'services/llm/openai_service.py': '333acc9e5e022d2341fcd9b07df46927eec34bd195c86abe3711073523468d61', 'services/calendar/contracts.py': '7b62912d0304849494597cad4323bab9c6b6303d41485a866762fd43b20d2a96', 'services/calendar/service_facts.py': '3e1c3a6977e9d008495e6bb312713b5544ed3fd03108597e61d520b3d53a6822', 'services/conversation/orchestrator.py': '40fc4cff4997fc6552621678483b3fe105e8d32f1cc860c2bcbdaf3a95b1caaf', 'services/scheduling/spoken_arabic.py': 'a11ad2a8bbd6e8c57e964fe717c37f7037b8d206c1b4e3d4d8c189ca3275d130', 'services/scheduling/proposals.py': 'df52bb23abd8f7855b6edb7e2b38a79c7cf13aae273c860cdc9f3369800c103b'}}
PROBE_PATH = "scripts/probe-booking-dialogue-repair.py"
PROBE_SHA = '70c3fe4abaaed30f017bdb9ea65979997690524e3c8dba77172def21875e0ab1'
...
def build_candidate(release, sources, old_image):
    ...
    parent = "ai-phone-booking-dialogue-repair-parent:" + release.directory.name
    tag = "ai-phone-booking-dialogue-repair:" + release.directory.name
    try:
...
def execute(release, helper):
    ...
    def probe(image_hashes, override=None):
        ...
        if override is None:
            ...
            name = "booking-dialogue-repair-probe-" + release.directory.name
            try:
    ...
    print("BOOKING_DIALOGUE_REPAIR_BASELINE_SOURCE_SETTINGS_READY_OK", flush=True)
    release.stage = "build"
    candidate = build_candidate(release, sources, old_image)
    print("BOOKING_DIALOGUE_REPAIR_LOCAL_PARENT_BUILD_OK", flush=True)
    release.stage = "speech-probe"
    name = "booking-dialogue-repair-parser-" + release.directory.name
    try:
        ...
        if b"BOOKING_DIALOGUE_OFFLINE_DAY_LANGUAGE_OK" not in result.stdout:
    ...
    print("BOOKING_DIALOGUE_REPAIR_CANDIDATE_SPEECH_AND_EFFECTIVE_SOURCE_OK", flush=True)
    ...
    try:
        ...
        try:
            ...
            print("BOOKING_DIALOGUE_REPAIR_PREVIOUS_APP_RESTORED", flush=True)
        except BaseException:
            release.force_close_ingress()
            print("BOOKING_DIALOGUE_REPAIR_RECOVERY_UNVERIFIED", flush=True)
        raise
    release.mark("deployed", (release.commit + "\n" + candidate + "\n").encode())
    print("BOOKING_DIALOGUE_REPAIR_DEPLOYED_READY_HTTPS_OK", flush=True)
...
def main(commit):
    ...
    with os.fdopen(fd, "r+") as lock:
        ...
        directory = Path(tempfile.mkdtemp(prefix="ai-phone-booking-dialogue-repair.", dir="/opt"))
        ...
        try:
            ...
            print("BOOKING_DIALOGUE_REPAIR_STOPPED_STAGE=" + release.stage, flush=True)
            raise
...
if __name__ == "__main__":
    try:
        ...
        print("BOOKING_DIALOGUE_REPAIR_STOPPED_KEEP_PROTECTED_FILES", flush=True)
